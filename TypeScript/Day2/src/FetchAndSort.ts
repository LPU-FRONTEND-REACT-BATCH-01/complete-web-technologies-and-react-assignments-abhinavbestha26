...
function processData(products: any[]) {

  let price5to10 = products.filter(
    (p) => p.price > 5 && p.price < 10
  );

  let priceAbove10 = products.filter(
    (p) => p.price > 10
  );

  let AtoZ = [...products].sort((a, b) =>
    a.title.localeCompare(b.title)
  );

  let ZtoA = [...products].sort((a, b) =>
